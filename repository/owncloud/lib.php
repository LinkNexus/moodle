...
class repository_owncloud extends repository {
    ...
    public function search($search_text, $page = 0, $path = '')
    {
        if (empty($path)) {
            $path = '/';
        }
        ...
        $ret = $this->get_listing_prepare_response($path);
        $results = $this->searchThroughFolders($path, $search_text, $page);
    ...
    public function searchThroughFolders($path, $search_text, $page): bool|array
    {
        // Before any WebDAV method can be executed, a WebDAV client socket needs to be opened
        // which connects to the server.
        $this->initiate_webdavclient();
        ...
        $list = $this->get_listing_convert_response($path, $ls);
        $searchResults = [];

        foreach ($list as $item) {
            if (is_array($item)) {
                if (str_contains($item['title'], $search_text)) {
                    $searchResults[] = $item;
                }

                if (array_key_exists('path', $item)) {
                    $searchResults = [...$searchResults, ...$this->searchThroughFolders($item['path'], $search_text, $page)];
                }
            }
        }
